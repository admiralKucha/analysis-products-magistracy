...
@anonymous
async def authentication(user: UserAuth, session: str = Cookie(default=None, include_in_schema=False)) -> Response:  # noqa: FAST002
    # авторизация
    res = await database_guest.authentication_user(user.model_dump())  # узнали, есть ли человек и забанен ли он
    code = res.pop("code")

    if res["status"] != "error":
        # пользователь прошел авторизацию
        user_id, max_age, role = res.pop("id"), res.pop("max_age"), res.pop("user_group")
        ...
        response.set_cookie("role", role, max_age=max_age)  # только для удобства фронтенду
